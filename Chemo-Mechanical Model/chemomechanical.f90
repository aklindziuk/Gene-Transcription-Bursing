...
a = .3
b = .07
kn = .001  !--Note: recording P(j), kn shouldnt be very large
kf = .002
y = 50  !--Note: when we're recording P(j) y shouldnt be 1
...
do i = 0,10
	print*, "P_j=",i, P(i)/dist_sum
	pf_check = pf_check + P(i)/dist_sum
end do

print*, 'P_off check', pf_check
...
open(9, file = "p_dist.dat", status = 'replace')
write(9,*), -1, p_on
do i = 0, 10
 	write(9,*),i, P(i)/dist_sum
end do 
close(9)
